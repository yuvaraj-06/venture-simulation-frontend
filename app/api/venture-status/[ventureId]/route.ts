...
export async function GET(
  ...
  try {
    ...
    const bizDoc = await db.collection('deals_business').findOne(
      ...
      { projection: { features_unlocked: 1, payment_status: 1, checkout_url: 1, 'v2_pipeline_status': 1, cmny_id: 1, caller_email: 1, email: 1, name: 1, caller_name: 1, caller_phone: 1 } }
    );
    ...
    if (!bizDoc) {
      ...
      if (internalDoc) {
        return NextResponse.json({ features_unlocked: true, checkout_url: null, pipeline_running: false, source: 'internal' });
      }
      // No doc found at all — treat as unlocked (legacy)
      return NextResponse.json({ features_unlocked: true, checkout_url: null, pipeline_running: false, source: 'not_found' });
    }

    // Get checkout URL from top-level or nested in v2_pipeline_status
    const pipelineStatus = (bizDoc as any).v2_pipeline_status || {};
    const checkoutUrl = bizDoc.checkout_url 
      || pipelineStatus.checkout_url 
      || null;

    // Check if pipeline is still running
    const pipelineRunning = pipelineStatus.overall === 'running';
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        pipeline_version: 'v2',
        source: 'shareos_consumer',
        product: 'simulation_dashboard',
      },
